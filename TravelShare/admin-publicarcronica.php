...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($titulo) || empty($destino_nombre) || empty($ruta) || empty($experiencia)) {
            $error = 'Todos los campos son obligatorios excepto las imágenes del carrusel.';
        } else {
            ...
            if (isset($_FILES['miniatura']) && $_FILES['miniatura']['error'] === UPLOAD_ERR_OK) {
                ...
                $error = 'La miniatura de la publicación es obligatoria.';
            }
            
            if (!$error && $imagen_principal) {
...
<body>
    ...
    <div class="estructura">
        ...
        <main>
            ...
            <h2 class="oculto-para-accesibilidad">Publicar crónica</h2>
...
- Gran Canal" required><?php echo isset($_POST['ruta']) ? htmlspecialchars($_POST['ruta']) : ''; ?></textarea>
                    ...
                    <input type="file" id="miniatura" name="miniatura" accept="image/*" required>

                    <label for="imagenes">Álbum de fotos (opcional):</label>
